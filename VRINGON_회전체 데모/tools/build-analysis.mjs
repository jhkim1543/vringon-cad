...
for (const s of idx.samples) {
  ...
  const body = { imageB64: "data:image/png;base64," + png.toString("base64"), dsl, ocrTokens: tokens.map((t) => ({ text: t.text, value: t.value, kind: t.kind })), partType, dims_read: ex?.dims_read || [] };
  const t0 = Date.now();
  try {
    ...
    writeFileSync(dir + "analysis.json", JSON.stringify({ ...j, part_type_inferred: partType, ocr_tokens: tokens.map((t) => t.text), built_at: new Date().toISOString() }, null, 2));
